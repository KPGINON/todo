...
4. 时间智能：相对时间（"明天下午3点""后天早上")需结合"当前时间"换算；模糊时间（"上午""下班前"）转 24 小制近似值（上午=09:00，下班前=18:00）；仅说"3点"默认下午 15:00；未提及时间为空。
...
- 顶层为对象，含 todos 数组字段，每个元素含：title（字符串）、priority（high/medium/low）、dueTime（HH:MM 或空字符串）、tags（字符串数组）、notes（字符串）。`;
...
{
  "todos": [
    { "title": "任务标题", "priority": "medium", "dueTime": "", "tags": [], "notes": "" }
  ]
}`;
  ...
  const data = await resp.json();
  const content = data.choices?.[0]?.message?.content || '';
  console.log('[parseTodo] AI raw content:', content);
  const json = extractJson(content);
  console.log('[parseTodo] parsed json:', JSON.stringify(json));
