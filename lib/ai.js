...
module.exports = { reschedule, parseTodo, suggestReorder, analyzeScreenshot, testConnection };
...
async function analyzeScreenshot(imageBase64, mimeType, existingTodos) {
  const { aiBaseUrl, aiApiKey, aiModel, visionModel } = store.getSettings();
...
请分析截图并输出排程 JSON。`;
  ...
  const resp = await fetch(url, {
    ...
    signal: AbortSignal.timeout(60000) // 图像分析给 60s，比文本更慢
  });
